...
class KeepassMerger:
    ...
    def add_entry(self, entry, db, debug=False):
        ...
        if parse_version(pykeepass_version) < parse_version("4.0.0"):
            ...
            self.addedAndModifiedEntries.append(string_path+'/'+new_entry.title)
    ...
    def sync_entries(self, db1, db2, debug=False):
        for entry in db1.entries:
            ...
            elif not entry.title in self.addedAndModifiedEntries:
                if entry.mtime > other_entry.mtime:
                    self.addedAndModifiedEntries.append(entry.title)
                    if debug:
    ...
    def merge_databases(self, file_input_databases,
                        file_output_database,
                        master_password,
        ...
        for file_db in itertools.islice(file_input_databases, 1, None):
            try:
                self.merge_two_databases(previous_file_db,
                                file_db,
                                file_output_database,
                                master_password,
                                debug)
